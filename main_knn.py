...
def cooccurrence_iterator(users, movies, batch_size, n_negs):
    ...
    while True:
        indices = torch.multinomial(counts, batch_size)
        yield rows[indices], cols[indices], torch.randint(0, M_mm.shape[0], (batch_size, n_negs))
    ...
    with tqdm.trange(iters_per_epoch) as t:
        for _ in t:
            ...
            pos_overlap = [
                    ...
                    for i, j in zip(I_q.numpy(), I_i.numpy())]
            neg_overlap = [
                    len(np.intersect1d(
                        HG.successors(i, etype='mu').cpu().numpy(),
                        HG.successors(j, etype='mu').cpu().numpy())) > 0
                    for i, j in zip(I_q.numpy(), I_neg[:, 0].numpy())]
            neg += sum(neg_overlap)
